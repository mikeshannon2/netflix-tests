...
TEST(NetflixFixture, check_exist_2) {
    string path = "/u/downing/cs/netflix-cs371p/took-forever-to-do-this-project.txt";
...
list<double> answers;
string path_to_actual_rating = "/u/downing/cs/netflix-cs371p/az6257-actual-user-ratings.txt";

TEST(NetflixFixture, get_answer_1) {
    string path = check_exist(path_to_actual_rating, "az6257-actual-user-ratings.txt");
    list<double> result = get_answer(path);
    answers.push_back(4);
    answers.push_back(4);
    answers.push_back(3);
    list<double>::iterator it1 = result.begin();
    for (list<double>::iterator it2 = answers.begin(); it2 != answers.end(); ++it2) {
        ASSERT_EQ(*it1, *it2);
        ++it1;
    }
}
...
TEST(NetflixFixture, get_answer_3) {
    ...
    for (list<double>::iterator it2 = reverse.begin(); it2 != reverse.end(); ++it2) {
        ASSERT_EQ(*it1, *it2);
        ++it1;
    }
}
...
// get_cache
// ---------
string path_to_avg_movie_rate = "/u/downing/cs/netflix-cs371p/sw32327-trc2346-movie-ratings.txt";
string path_to_avg_customer_rate = "/u/downing/cs/netflix-cs371p/sw32327-trc2346-customer-ratings.txt";

TEST(NetflixFixture, get_cache_1) {
    string path = check_exist(path_to_avg_movie_rate, "sw32327-trc2346-movie-ratings.txt");
...
TEST(NetflixFixture, get_cache_3) {
    string path = check_exist(path_to_avg_customer_rate, "sw32327-trc2346-customer-ratings.txt");
...
TEST(NetflixFixture, read_1) {
    ...
    for (list<int>::iterator it = movie_ids.begin(); it != movie_ids.end(); ++it) {
        k >> s;
...
TEST(NetflixFixture, get_prediction_1) {
    ...
    path = check_exist(path_to_avg_movie_rate, "sw32327-trc2346-movie-ratings.txt");
    avg_movie = get_cache(path);
    path = check_exist(path_to_avg_customer_rate, "sw32327-trc2346-customer-ratings.txt");
